...
**退院の手続き**プレゼンテーションを作成します。このプレゼンテーションも **手術前の手続き** と同様、**スタート**を基に作成します。作成したプレゼンテーションに、チェック・ボックスを追加し、下表に従ってプロパティを設定します。姓名は読み取り専用にしておきます。
...
3. Repeatable（反復可能）の右側にある鉛筆アイコンをクリックし、Data Driven（データ・ドリブン）の右にある **＋** をクリックし、継続条件を追加します。今回は、フォームの退院判定プレゼンテーションの[退院可]チェックボックスの値で判定させるために、 **formArg.isTreatmentCompleted** が **false** であることを継続条件とします。設定完了後、Save（保存）をクリックします。
    ...
    <img src="https://raw.githubusercontent.com/anishi1222/IntegrationCloud/images/DynamicProcess-Tutorial/image43-1.png" style="width:2.00000in;height:2.5987in"/>
